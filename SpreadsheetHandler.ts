...
namespace SpreadsheetHandler {
    ...
    export function loadArticles(count: number): Article[] {
        ...
        return rows.map(row => convertRowToArticle(row)).filter(x => x)
    }
    ...
    export function logSending(articles: Article[]): void {
        const rowIndex = getFirstEmptyRowIndex(logSheet.getRange('A:A').getValues())
    ...
    export function addSubscription(emailAddress: string, hungarianPosts: boolean, englishPosts: boolean, articleSrs: boolean): number {
    ...
    function getFirstEmptyRowIndex(values): number {
        let rowIndex = 0
